 ...
 * 单一循线应用入口。
 *
 * 职责：按安全顺序初始化硬件并调度 6.667 ms 控制任务。
 * 依赖：SysConfig、motor、line_run。
 * 上下文：main 主循环；TIMG0 ISR 只递增 tick。
 * 数据单位：gControlTick 每单位 6.667 ms。
 * 硬件副作用：初始化外设、开启 PWM 计数器（比较值为 0）和 TIMG0。
 * 参数无效时保持停车，不启动定时控制。
 */
...
static void App_HandleKeys(void)
{
    ...
    params = ControlParams_Get();
    if (params == 0) {
        return;
    }

    if (key2Event) {
        (void) LineRun_StartStraightDistance(params->straightDistanceCounts);
        return;
    }

    if (key3Event) {
        (void) LineRun_StartLineDistance(params->lineLapDistanceCounts);
    }
}
