...
static bool LineRun_StartMode(LineRunMode mode, int32_t distanceCounts);
...
bool LineRun_Start(void)
{
    ...
    return LineRun_StartLineDistance(gParams->lineLapDistanceCounts);
}

bool LineRun_StartStraightDistance(int32_t distanceCounts)
{
    return LineRun_StartMode(LINE_RUN_MODE_STRAIGHT, distanceCounts);
}

bool LineRun_StartLineDistance(int32_t distanceCounts)
{
    return LineRun_StartMode(LINE_RUN_MODE_LINE, distanceCounts);
}

bool LineRun_StartBench(void)
{
    if (!LineRun_StartMode(LINE_RUN_MODE_BENCH, 0)) {
        return false;
    }
    gBenchTarget = LINE_RUN_BENCH_DEFAULT_TARGET;
    return true;
...
{
    if (!gRunning || (gMode != LINE_RUN_MODE_BENCH) ||
        (target < 1) || (target > 8)) {
        return false;
    }
    gBenchTarget = target;
...
{
    return gRunning && (gMode == LINE_RUN_MODE_BENCH);
}
...
static bool LineRun_StartMode(LineRunMode mode, int32_t distanceCounts)
{
    ...
    if (((mode == LINE_RUN_MODE_LINE) || (mode == LINE_RUN_MODE_STRAIGHT)) &&
        (distanceCounts <= 0)) {
        return false;
    }
    LineRun_ResetControlState();
    ...
    gTargetDistanceCounts = distanceCounts;
    gBenchTarget = LINE_RUN_BENCH_DEFAULT_TARGET;
    gMode = mode;
    gRunning = true;
    return true;
...
static bool LineRun_HasReachedDistance(void)
{
    ...
    if ((gMode != LINE_RUN_MODE_LINE) &&
        (gMode != LINE_RUN_MODE_STRAIGHT)) {
        return false;
    }
    leftDelta = LineRun_AbsI32(Encoder_GetLeftCount() - gStartLeftCount);
    rightDelta = LineRun_AbsI32(Encoder_GetRightCount() - gStartRightCount);
