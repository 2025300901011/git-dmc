...
static bool ControlParams_IsValid(const ControlParams *params)
{
    ...
    if ((params->speedIntegralLimit < 0) ||
        ...
        (params->straightDistanceCounts <= 0) ||
        (params->straightDistanceCounts > PARAM_DISTANCE_COUNTS_MAX) ||
        (params->lineLapDistanceCounts <= 0) ||
        (params->lineLapDistanceCounts > PARAM_DISTANCE_COUNTS_MAX)) {
        return false;
    }
